...
mydraw<-runif(n_draw)
print(paste0("Theoretical number of duplicate is ",length(mydraw)*est_num_expected_duplicate(n_draw,n_unique_double)))
print(paste0("The number of duplicates from my draw is ",length(mydraw)-length(unique(mydraw))  ))
```
There is difference between the number of duplicates in the generated sequence and the theoretical value, but the difference is small. Partially because there is randomness in the random sampling, the comparison between one time draw and theoretical value may not be reasonable. I may expect if I do the draw many times, it will vary around the theoretical value. 
...
#### I expect the line will be a straight line with negative slope. So, this output does not agree with my expectation. The reason is consistent with what we learn from class about the computation of exp in R. When the dx is too small, the difference between two func values cannot capture their real difference.
...
summary(reg)
```
#### Therefore, the slope of the latter part is 1. 
...
auto_choose<-function(func,x){
  log2_dx <- - seq(1, 52)
  iter_<-1
  
  old_num_derive<-approximate_deriv(func, x, 2^log2_dx[iter_])
  old_change<-NULL
  iter_<-iter_ + 1
  continous_sign<-TRUE
  while (continous_sign) {
    cur_dx<-log2_dx[iter_]
    cur_num_derive<-approximate_deriv(func, x, 2^cur_dx)
    cur_change<-(cur_num_derive-old_num_derive)/old_num_derive
    if(!is.null(old_change)){
     if(sign(cur_change) != sign(old_change)){
      continous_sign<-FALSE
      break
      } 
    }
    old_dx <-cur_dx
    old_num_derive<-cur_num_derive
    old_change<-cur_change
    iter_ <- iter_ + 1
    if(iter_== length(log2_dx)){
      print("Reach minimal dx!")
      break
    }
  }
  return(list("log2_dx" = old_dx,"num_derivative" = old_num_derive))
}

auto_choose(log_logistic_func,x = x)
```
